...
class Simulator:
        ...
        while not done:
            ...
            if updateQ:
                self.agent.update_values(
                    state, action, reward, next_state, prob_actions
                )
    ...
    def update_agent_noise(self):
        ...
        grad_cost = (
            self.agent.sigma / (self.agent.sigma_base**2) - 1 / self.agent.sigma
        )
        grad_mean = np.mean(grads, axis=0)

        # Updating sigmas
        self.agent.sigma += self.agent.learning_sigma * (
            grad_mean - self.agent.lmda * grad_cost
        )
    ...
    def compute_cost(self) -> float:
        ...
        S0 = np.diag(
            np.square(np.ones(len(self.agent.sigma.flatten())) * self.agent.sigma_base)
        )
